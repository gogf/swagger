...
// WrapHandler wraps `http.Handler` into `ghttp.HandlerFunc`.
func WrapHandler(h *webdav.Handler, configs ...func(c *Config)) ghttp.HandlerFunc {
...
// CustomWrapHandler wraps `http.Handler` into `ghttp.HandlerFunc`
func CustomWrapHandler(config *Config, h *webdav.Handler) ghttp.HandlerFunc {
	...
	return func(r *ghttp.Request) {
		...
		if matches = rexp.FindStringSubmatch(r.RequestURI); len(matches) != 3 {
			...
			r.Response.Write("404 page not found")
			return
		}
		path := matches[2]
		prefix := matches[1]
...
func DisablingWrapHandler(h *webdav.Handler, envName string) ghttp.HandlerFunc {
...
// if specified environment variable passed
func DisablingCustomWrapHandler(config *Config, h *webdav.Handler, envName string) ghttp.HandlerFunc {
	eFlag := os.Getenv(envName)
	if eFlag != "" {
		return func(r *ghttp.Request) {
			// Simulate behavior when route unspecified and
